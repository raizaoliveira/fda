...
def readDead():
    ...
    for file in glob.glob("*.csv"):
        ...
        for point in reader:
            ...
            print(">>>>> i == ", i)
            if i == 0:
                print ("Analise ", line[0],"DEPENDENCIAS NOVAS", line[1], "Variabilidades: ", line[2] )
                version.setVersion(line[0])
                version.setVariabilitiesNumber(line[2])
                version.setNewDependencies(line[1])
            if i > 0:
                if ( int(version.getNewDependencies()) > 0):
                    #print (linha)
                    print("<<<<< i == ", i)
                    if "Dependency" in line:
                        if (dp):
                            version.addNewDependency(lastDependency)
...
def process(project):
    ...
    analyzedLinks = list()   
    with open("NewDependenciesReport.csv", "w") as csv_file:
        writer = csv.writer(csv_file, delimiter=',')
        for i in range (0, len(allVersions) - 1):
            for dp in allVersions[i].getNewDependency():
                for link in dp.getNewLinks():
